...
- 提供更用户友好、更低门槛的文件管理方式，可直接使用自带的文件管理界面，也可远程登录云服务器或直接操作电脑
...
- `workbuddyExePath`：可选，指定 `WorkBuddy.exe` 的绝对路径；留空时优先查找和 `workbuddy-remote` 平级的 `WorkBuddy\WorkBuddy.exe`
- `workbuddyUserDataDir`：可选，指定 WorkBuddy 启动时传入的 `--user-data-dir`；留空时默认使用 `workbuddy-remote\output\runtime\workbuddy-user-data`
- `runtimeRootDir`：可选，指定 bridge 运行时目录；bridge 日志、缓存、临时文件等会写到这里；留空时默认使用 `workbuddy-remote\output\runtime`
...
- `workspaceRoots`：可选，指定允许访问的工作区根目录列表；留空时默认使用 `C:\Users\<当前用户>\WBWorkspaces` 和 `C:\Users\<当前用户>\WorkBuddy`
- `maskBridgeModelSecrets`：是否在 WorkBuddy-Remote 的浏览器页面内隐藏 API 地址和 API Key 输入框内容

现在默认不会再把 bridge 的 `tmp`、日志等运行时文件写进 `WorkBuddy` 安装目录。
...
默认情况下，文件管理器使用 `C:\Users\<当前用户>\WBWorkspaces` 和 `C:\Users\<当前用户>\WorkBuddy`。为了确保安全，文件管理器被设置为仅能操作工作区根目录下的文件。
